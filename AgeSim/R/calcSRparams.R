calcSRparams <- function(ts){
  ...
  # Phase 1 
  obj <-MakeADFun(df.tmb,parms,DLL="recruitment", silent = TRUE)
  tt <- obj$report()
  ...
  plot(SSB,R, xlim = c(0,max(SSB, na.rm = TRUE*1.1)), ylim = c(0, max(R, na.rm =TRUE)*1.1))
  lines(SSBpred,Rpred)
  }
